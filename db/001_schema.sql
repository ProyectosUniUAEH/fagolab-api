...
-- Una secuenciación puede nacer de una reacción PCR, de un carril de gel candidato o de
-- un vial; también puede ser una demostración con datos públicos de NCBI o sintéticos,
-- y entonces no tiene cadena experimental propia. `origen_dato` es obligatorio: el
-- sistema nunca debe confundir un dato del laboratorio con uno descargado de internet.
CREATE TABLE secuenciaciones (
  ...
  id_pcr_reaccion UUID REFERENCES pcr_reacciones(id_pcr_reaccion) ON DELETE RESTRICT,
  id_gel UUID REFERENCES geles_electroforesis(id_gel) ON DELETE SET NULL,
  id_carril_gel UUID REFERENCES carriles_gel(id_carril_gel) ON DELETE SET NULL,
  id_vial_adn UUID REFERENCES viales_adn(id_vial_adn) ON DELETE SET NULL,
  origen_dato TEXT NOT NULL DEFAULT 'experimental'
    CHECK (origen_dato IN ('experimental','publico_ncbi','sintetico')),
  fuente_externa TEXT,
  accession_externo TEXT,
  organismo_declarado TEXT,
  plataforma TEXT,
  tecnologia TEXT,
  layout TEXT,
  laboratorio TEXT,
  fecha_secuenciacion DATE,
  notas_procedencia TEXT,
  proveedor TEXT,
...
CREATE INDEX idx_secuenciaciones_origen ON secuenciaciones(origen_dato, estado_secuenciacion);
CREATE INDEX idx_secuenciaciones_gel ON secuenciaciones(id_gel);

-- FASTQ = lecturas crudas del secuenciador (secuencia + calidad).
-- FASTA = secuencia ya procesada (consenso / contigs), sin calidad.
-- Validación y QC son deterministas (sin IA) y se guardan como métricas JSONB.
CREATE TABLE archivos_secuencia (
...
CREATE INDEX idx_archivos_secuencia_sec ON archivos_secuencia(id_secuenciacion, created_at);

-- Evidencia externa (BLAST, taxonomía NCBI, PubMed) con fecha de consulta y hash: una
-- ficha generada por el agente debe poder rastrearse hasta la evidencia que la sustentó.
CREATE TABLE evidencias_externas (
...
CREATE INDEX idx_evidencias_secuenciacion ON evidencias_externas(id_secuenciacion, tipo);
CREATE INDEX idx_evidencias_pmid ON evidencias_externas(pmid) WHERE pmid IS NOT NULL;

-- La ficha científica: la única salida del pipeline generada por el modelo. Guarda cómo y
-- con qué se generó, para poder reproducirla, compararla y auditar en qué se apoyó.
CREATE TABLE fichas_analisis (
...
CREATE INDEX idx_fichas_secuenciacion ON fichas_analisis(id_secuenciacion, created_at DESC);
...
-- Trabajo que tarda minutos y no cabe en un request HTTP: BLAST devuelve un RID que hay
-- que consultar hasta que termina. Por eso la corrida es un objeto con estado propio.
CREATE TABLE corridas_analisis (
  id_corrida UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tipo_analisis TEXT NOT NULL,
  herramienta TEXT,
  id_secuenciacion UUID REFERENCES secuenciaciones(id_secuenciacion) ON DELETE CASCADE,
  id_archivo_secuencia UUID REFERENCES archivos_secuencia(id_archivo_secuencia) ON DELETE SET NULL,
  nombre_modelo TEXT,
  version_modelo TEXT,
...
CREATE INDEX idx_corridas_secuenciacion ON corridas_analisis(id_secuenciacion, created_at DESC);
